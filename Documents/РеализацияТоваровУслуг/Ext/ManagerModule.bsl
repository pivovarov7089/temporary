...
Функция Расш1_СформироватьПечатнуюФормуНакладная(МассивОбъектов, ОбъектыПечати, ПараметрыПечати) Экспорт 
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Пока ВыборкаПоТоварам.Следующий() Цикл
			...
			Если Не НаборыСервер.ИспользоватьОбластьКомплектующие(ВыборкаПоТоварам, ИспользоватьНаборы)
				И Не НаборыСервер.ВыводитьТолькоЗаголовок(ВыборкаПоТоварам, ИспользоватьНаборы) Тогда
				Если Расш1_УпаковкаСчитаетсяКоробкой(ВыборкаПоТоварам) Тогда
					ИтогоКоробок = ИтогоКоробок + ВыборкаПоТоварам.Количество;
				ИначеЕсли Расш1_СчитатьКакОднуКоробПоСоотношениюУпаковки(ВыборкаПоТоварам) Тогда
					ИтогоКоробок = ИтогоКоробок + ВыборкаПоТоварам.Количество;
				Иначе
					ПоБазе = ВыборкаПоТоварам.КоличествоВБазовыхЕдиницах;
					Если ПоБазе = 0
						И ЗначениеЗаполнено(ВыборкаПоТоварам.Количество) Тогда
						ПоБазе = ВыборкаПоТоварам.Количество; // в уп. — резерв, если в базе нули
					КонецЕсли;
					ИтогоШтук = ИтогоШтук + ПоБазе;
				КонецЕсли;
			КонецЕсли;
		...
		ТекстИтогоКорШтВКолонке = Расш1_ПредставлениеИтогоКоробокИШтук(ИтогоКоробок, ИтогоШтук);
		УстановитьПараметр(ОбластьДанныеКоличество, "Количество", СокрЛП(ТекстИтогоКорШтВКолонке));
...
// Итог «N кор M шт» в подвале по колонке «Количество»; коробка — по подстроке в упаковке/ед.изм.
//
Функция Расш1_УпаковкаСчитаетсяКоробкой(СтрокаТовара)
	ТекстУп = СокрЛП(Строка(СтрокаТовара.Упаковка));
	ТекстЕд = СокрЛП(Строка(СтрокаТовара.ЕдиницаИзмерения));
	Сборка = ВРег(ТекстУп + " " + ТекстЕд);
	Возврат СтрНайти(Сборка, "КОР") > 0
		Или СтрНайти(Сборка, "КОРОБ") > 0;
КонецФункции

// Когда в справочнике нет «кор» в названии упаковки, в строке 1 крупная упр. и много «шт» (тип. короб 12/24).
//
Функция Расш1_СчитатьКакОднуКоробПоСоотношениюУпаковки(СтрокаТовара)
	Если Не (СтрокаТовара.Количество = 1) Тогда
		Возврат Ложь;
	КонецЕсли;
	ВБазе = СтрокаТовара.КоличествоВБазовыхЕдиницах;
	Если ВБазе <= 1
		Или ВБазе < 4 Тогда
		Возврат Ложь;
	КонецЕсли;
	// Уже поймали «кор» в тексте — путь в ИтогоКоробок через Расш1_УпаковкаСчитаетсяКоробкой
	Возврат Не Расш1_УпаковкаСчитаетсяКоробкой(СтрокаТовара);
КонецФункции

Функция Расш1_ПредставлениеИтогоКоробокИШтук(Кор, Шт)
	Если Кор > 0 И Шт > 0 Тогда
		Возврат СтрШаблон(НСтр("ru = '%1 кор %2 шт'"), Формат(Кор, "ЧН=0; ЧГ=0"), Формат(Шт, "ЧН=0; ЧГ=0"));
	ИначеЕсли Кор > 0 Тогда
		Возврат СтрШаблон(НСтр("ru = '%1 кор'"), Формат(Кор, "ЧН=0; ЧГ=0"));
	ИначеЕсли Шт > 0 Тогда
		Возврат СтрШаблон(НСтр("ru = '%1 шт'"), Формат(Шт, "ЧН=0; ЧГ=0"));
	КонецЕсли;
	Возврат "";
КонецФункции
