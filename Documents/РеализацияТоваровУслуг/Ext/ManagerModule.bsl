...
Функция Расш1_СформироватьПечатнуюФормуНакладная(МассивОбъектов, ОбъектыПечати, ПараметрыПечати) Экспорт 
	...
	|		КОГДА ВложенныйЗапрос.Упаковка <> ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
	|				И (ВЫРАЗИТЬ(ВложенныйЗапрос.Упаковка.Наименование КАК СТРОКА(150))) ПОДОБНО ""%кор%""
	|			ТОГДА Формат(ВложенныйЗапрос.КоличествоУпаковок, ""ЧН=0; ЧГ=0"") + "" кор ("" + Формат(ВложенныйЗапрос.Количество, ""ЧН=0; ЧГ=0"") + "" шт)""
	|		КОГДА ВложенныйЗапрос.Упаковка = ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
	|			ТОГДА Формат(ВложенныйЗапрос.Количество, ""ЧН=0; ЧГ=0"") + "" "" + (ВЫРАЗИТЬ(ВложенныйЗапрос.ЕдиницаИзмерения.Наименование КАК СТРОКА(50)))
	|		ИНАЧЕ Формат(ВложенныйЗапрос.КоличествоУпаковок, ""ЧН=0; ЧГ=0"")
