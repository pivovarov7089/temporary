	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Пока ВыборкаПоТоварам.Следующий() Цикл
			...
			Если Не НаборыСервер.ИспользоватьОбластьКомплектующие(ВыборкаПоТоварам, ИспользоватьНаборы)
				...
				ИмяУпаковкиВРег = ВРег(СокрЛП(Строка(ВыборкаПоТоварам.Упаковка)));
				Если СтрНайти(ИмяУпаковкиВРег, "КОР") > 0 Тогда
		...
		ТекстКорШт = "";
		Если ИтогоКоробок > 0 И ИтогоШтук > 0 Тогда
			ТекстКорШт = СтрШаблон(НСтр("ru = '%1 кор %2 шт'"), Формат(ИтогоКоробок, "ЧН=0; ЧГ=0"), Формат(ИтогоШтук, "ЧН=0; ЧГ=0"));
		ИначеЕсли ИтогоКоробок > 0 Тогда
			ТекстКорШт = СтрШаблон(НСтр("ru = '%1 кор'"), Формат(ИтогоКоробок, "ЧН=0; ЧГ=0"));
		ИначеЕсли ИтогоШтук > 0 Тогда
			ТекстКорШт = СтрШаблон(НСтр("ru = '%1 шт'"), Формат(ИтогоШтук, "ЧН=0; ЧГ=0"));
		КонецЕсли;
		Если Не ПустаяСтрока(ТекстКорШт) Тогда
			ИтоговаяСтрока = ИтоговаяСтрока + Символы.ПС + ТекстКорШт;
		КонецЕсли;
