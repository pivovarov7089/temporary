...
Функция Расш1_СформироватьПечатнуюФормуНакладная(МассивОбъектов, ОбъектыПечати, ПараметрыПечати) Экспорт 
	...
	|		КОГДА ВложенныйЗапрос.Упаковка <> ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
	|				И (ВЫРАЗИТЬ(ВложенныйЗапрос.Упаковка.Наименование КАК СТРОКА(150))) ПОДОБНО ""%кор%""
	|			ТОГДА ВЫРАЗИТЬ(ВЫРАЗИТЬ(ВложенныйЗапрос.КоличествоУпаковок КАК ЧИСЛО(15, 0)) КАК СТРОКА(15))
	|				+ "" кор (""
	|				+ ВЫРАЗИТЬ(ВЫРАЗИТЬ(ВложенныйЗапрос.Количество КАК ЧИСЛО(15, 0)) КАК СТРОКА(15))
	|				+ "" шт)""
	|		КОГДА ВложенныйЗапрос.Упаковка = ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
	|			ТОГДА ВЫРАЗИТЬ(ВЫРАЗИТЬ(ВложенныйЗапрос.Количество КАК ЧИСЛО(15, 0)) КАК СТРОКА(15))
	|				+ "" ""
	|				+ (ВЫРАЗИТЬ(ВложенныйЗапрос.ЕдиницаИзмерения.Наименование КАК СТРОКА(50)))
	|		ИНАЧЕ ВЫРАЗИТЬ(ВЫРАЗИТЬ(ВложенныйЗапрос.КоличествоУпаковок КАК ЧИСЛО(15, 0)) КАК СТРОКА(15))
